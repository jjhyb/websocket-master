...
function connect(event) {
    ...
    if(username) {
        ...
        // 加入 token 头信息
        stompClient.connect({'token': '456'}, onConnected, onError);
...
function onConnected() {
    // 订阅群发主题
    stompClient.subscribe('/exchange/WebSocket/*',onMessageReceived,{ack:'client'})

    // 订阅一对一主题，即通过用户名等唯一性标识拼接到订阅主题地址
    stompClient.subscribe('/topic/single' + username,onMessageReceived)

    // stompClient.subscribe('/amq/queue/fanoutQueue', onMessageReceived,{ack:'client'});
